...
test("release workflow publishes the approved candidate without rebuilding", async () => {
  ...
  assert.equal(parsed.jobs.readiness.if, "github.ref == 'refs/heads/main'");
  assert.equal(parsed.jobs.candidate.uses, "./.github/workflows/release-candidate.yml");
  assert.equal(parsed.jobs.candidate.with.ref, "${{ github.sha }}");
  assert.equal(parsed.jobs.runtime.needs, "candidate");
  assert.deepEqual(parsed.jobs.runtime["runs-on"], ["self-hosted", "harnessbrew-runtime"]);
  assert.deepEqual(parsed.jobs.evidence.needs, ["candidate", "runtime"]);
  assert.deepEqual(parsed.jobs.publish.needs, ["candidate", "evidence"]);
  assert.equal(Object.values(parsed.jobs).filter((job: any) => job.environment).length, 1);
  assert.match(workflow, /scripts\/release\/evidence.mjs/u);
  assert.match(workflow, /scripts\/release\/publish.mjs/u);
  assert.doesNotMatch(workflow, /candidate-run-id|workflow_run|pull_request|secrets: inherit/u);
  assert.equal(parsed.jobs.publish.permissions["id-token"], "write");
  assert.equal(parsed.jobs.runtime.permissions["id-token"], undefined);
...
test("release candidate workflow builds once and retains cross-platform evidence", async () => {
  ...
  assert.match(workflow, /name: \$\{\{ needs.build.outputs.artifact \}\}-gate-\$\{\{ runner.os \}\}/u);
  const parsed = parse(workflow);
  assert.ok(parsed.on.workflow_call);
  assert.equal(parsed.jobs.gate.needs, "build");
  assert.equal(parsed.on.workflow_call.outputs.artifact.value, "${{ jobs.build.outputs.artifact }}");
  const download = parsed.jobs.gate.steps.find((step: any) => step.uses === "actions/download-artifact@v8");
  assert.equal(download.with.name, "${{ needs.build.outputs.artifact }}");
