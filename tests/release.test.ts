...
test("release source metadata is synchronized", async () => {
  const result = await execFileAsync(process.execPath, [releaseCheck, "v0.6.1"], { encoding: "utf8" });

  assert.match(result.stdout, /harnessbrew@0\.6\.1 \(v0\.6\.1\)/);
});
...
test("release source verification rejects a mismatched tag", async () => {
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      assert.match(error.message, /release tag v0\.5\.2 must match v0\.6\.1/);
      return true;
    }
  );
});
