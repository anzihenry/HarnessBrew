...
const deliverables = [
  ...
  { kind: "instruction", name: "runtime-instruction", operation: "managed-block" },
...
function placementArgs(environment, target, scope) {
  return scope === "user"
    ? ["--scope", "user", "--target-root", environment.paths.codexRoot]
...
export async function targetMatrixScenario({ environment, cli }) {
  ...
  for (const formula of deliverables) {
    ...
    assert.equal(receipt.operations.length, 2, `${formula.kind} should record two placements`);
    assert.deepEqual([...new Set(receipt.operations.map((operation) => operation.target))].sort(), targets.slice().sort());
    for (const operation of receipt.operations) {
      const expected = formula.operation;
